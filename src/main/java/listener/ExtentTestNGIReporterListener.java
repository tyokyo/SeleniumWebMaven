...
public class ExtentTestNGIReporterListener implements IReporter {
	...
	private void init() {
		//文件夹不存在的话进行创建
		File reportDir= new File(this.output_folder_dir);
		if(!reportDir.exists()&& !reportDir .isDirectory()){
			reportDir.mkdir();
		}
		/*ExtentHtmlReporter htmlReporter = new ExtentHtmlReporter(OUTPUT_FOLDER + FILE_NAME);
	    htmlReporter.config().setDocumentTitle(utils.ReportUtil.getReportName());
        htmlReporter.config().setReportName(utils.ReportUtil.getReportName());
